...
_ISSUE_DETECTION_RULES: dict[str, tuple[tuple[str, ...], tuple[str, ...]]] = {
    ...
    "appellate_reversal_issue": (
        (
            ...
            "рассмотрено в отсутствие не извещенного",
            "решение подлежит отмене",
            "апелляция из-за нарушения процедуры",
            "обжалование по процессуальным нарушениям",
            # Case-specific: direct "appeal/challenge the judgment" intent
            "обжаловать решение",
            "как обжаловать",
    ...
    "foreign_service_issue": (
        (
            "по чешскому адресу",
            "по иностранному адресу",
            "вручение за рубежом",
            "доставка документов за границу",
            "международное вручение",
            "в иностранное государство",
            "международная правовая помощь",
            "международную правовую помощь",
            "международной правовой помощью",
            "международной правовой помощи",
            "не направили за рубеж",
            # Case-specific: Czech address forms without "по ... адресу" wrapper
            "чешскому адресу",
            "чешском адресе",
            "по моему чешскому адресу",
            "иностранному адресу",
        ),
        ("загран", "зарубеж"),
    ),
    "alimony_exemption_issue": (
        (
            "освобождение от задолженности по алиментам",
            "уважительные причины неуплаты",
            "не знал о решении суда",
            "не мог платить по уважительной причине",
            "освобождение от долга по алиментам",
        ),
        ("освобожд", "уважит"),
    ),
    "missed_deadline_due_to_service_issue": (
        (
            ...
            "пропустил срок на подачу жалобы",
            "восстановление срока",
            "восстановить процессуальный срок",
            "срок пропущен из-за ненадлежащего извещения",
            "не знал о решении и пропустил срок",
            "пропущен срок по причине отсутствия извещения",
            # Case-specific: "восстановить" infinitive form (stem "восстановл" does not match)
            "восстановить срок",
            "восстановить срок на апелляцию",
            "восстановить срок на обжалование",
        ),
        ("восстановл", "восстанови", "пропущ"),
    ),
    "recognition_enforcement_issue": (
        (
            ...
            "отказ в признании иностранного решения",
            "отказ в признании иностранного судебного решения",
            "отказ в исполнении иностранного решения",
            "решение чешского суда",
            "чешское судебное решение",
...
_ISSUE_TO_TOPICS: dict[str, tuple[str, ...]] = {
    ...
    "appellate_reversal_issue": ("appellate_reversal",),
    "foreign_service_issue": ("foreign_service_procedure",),
    "alimony_exemption_issue": ("alimony_debt",),
    "missed_deadline_due_to_service_issue": ("procedural_deadline_restoration",),
    "recognition_enforcement_issue": ("recognition_enforcement",),
}
