...
def _evaluate_claim(law: str, article: str, claim: str, article_text: str) -> tuple[str, str]:
    ...
    # Targeted legal sanity rules for this case
    if "гпк" in law.lower() and article == "329" and ("service" in low_claim or "notice" in low_claim):
        return (
            "SUSPECT",
            "GPK 329 is mainly about appellate ruling/notification form, not core foreign service doctrine.",
        )
    if "гпк" in law.lower() and article == "438" and ("appeal deadline" in low_claim or "апел" in low_claim):
        return (
            "MISMATCH",
            "GPK 438 concerns enforcement proceedings; using it for appeal-deadline restoration is doctrinally wrong.",
        )
    if "екпч" in law.lower() and article == "5" and "civil" in low_claim:
        return (
            "SUSPECT",
            "ECHR Art. 5 mainly concerns liberty/arrest, weak support for civil notice/service disputes.",
        )
...
def run(agent_output_log: Path, corpus_root: Path) -> int:
    ...
    print(f"Detected GPK file: {law_files['гпк']}")
    print(f"Detected ECHR file: {law_files['екпч']}")
    print()
